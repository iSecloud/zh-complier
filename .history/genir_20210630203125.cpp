...
Var* GenIR::genCall(Fun* fun, vector<Var*>& args)
{
    ...
    else
    {
        ...
        symtab.addVar(tmp); //Pro 将返回值声明延迟到函数调用之后
        return tmp;
    }
}
...
Var* GenIR::genSigOpRight(Tag opt, Var* var)
{
    ...
    if(opt == INC) return genIncRight(var);
...
bool GenIR::genVarInit(Var* var)
{
    if(var->getName()[0] == '<') 
        return false;
    symtab.addCode(new Quaternion(OP_DEC, var)); //声明变量
    if(var->setInit()) //局部变量初始化
        genBinOp(ASSIGN, var, var->getInit()); 
    return true;
}