...
void SymTab::printInterInfo(int isopt)
{
    if(!isopt) 
        printf("\n-----------Unoptimized code----------\n");
    else 
...
void SymTab::genAssemble(FILE* file, bool isopt)
{
    genSegment(file); //生成数据段
    fprintf(file, ".text\n"); //生成代码段
    for(auto f: funTab)
    {
        Fun* fun = f.second;
        fun->getObjCode(file, isopt); //生成汇编代码
    }
}
