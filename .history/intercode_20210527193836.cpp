...
void Quaternion::toString()
{
    ...
    else if(op == OP_PROC) printf("OP_PROC call function %s", fun->getName());
    else if(op == OP_CALL) printf("OP_CALL "), result->info(), printf("=%s", fun->getName());
    else if(op == OP_RET) printf("OP_RET return point is %s", label);
    else if(op == OP_RETX) printf("OP_RETX return value is"), arg1->info(), printf(" point is %s", label);
