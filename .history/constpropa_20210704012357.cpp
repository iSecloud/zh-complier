...
void ConstPropagation::algSimple()
{
    ...
    {
        //得到当前数据块的指令流, 进行代数化简
        if(block == dfg->blocks[0]) continue; //跳过entry
        //printf("in %d out %d", block->inVals.size(), block->outVals.size()); 
        //system("pause");
        ...
        for(i = block->blockCode.begin(), k = i; i != block->blockCode.end(); i = k)
        {
            ...
            if(op == OP_JF || op == OP_JT || op == OP_JNE)
            {
                ...
                if((op == OP_JT && !cond) || (op == OP_JF && cond))
                {
                    ...
                    //code->getBlock()->blockCode.remove(code);
                ...
                else if(op == OP_JNE)
                {
                    Var* arg2 = code->getArg2();
                    double cond2;
                    if(arg2->isConst()) cond2 = arg2->getVal();
                    else cond2 = code->outVals[arg2->index];
                    if(cond2 == UNDEF || cond2 == NAC) continue;
                    if(cond1 != cond2)
                    {
                        code->replace(OP_JMP, target);
                        if(target->getBlock() != dfg->blocks[bk + 1])
                        d   fg->delBlock(block, dfg->blocks[bk + 1]);
                    }
                }
            }
        }
        //dfg->blocks[bk]->printCode(); system("pause");
    }
}
