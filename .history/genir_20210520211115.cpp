...
Var* GenIR::genSigOp(Tag opt, Var* var)
{
    ...
    if(var->getType() == KW_VOID)
    {
        Error::showError(CALC_VAL_ERR);
        return NULL;
    }
    if(opt == OP_LEA) return genAddress(var);
    if(opt == OP_MUL) return genPtr(var);
    if(opt == OP_INCV) return genIncv(var);
    if(opt == OP_DECV) return genDecv(var);
    if(var->getPointer() != NULL) var = genVal(var);
    if(opt == OP_NEG) return genNeg(var);
    if(opt == OP_NOT) return genNot(var);
    if(opt == OP_INV) return genInv(var);
}