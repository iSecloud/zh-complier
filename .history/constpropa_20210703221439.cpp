...
void ConstPropagation::transferVar(Quaternion* code, vector<double> &in, vector<double> &out)
{
    ...
    {
        if(op == OP_ADD) printf("this is add"), system("pause");
        ...
        if(op == OP_ADD) printf("this is add2"), system("pause");
    }
    //GET 保守认为x = *y x会变
